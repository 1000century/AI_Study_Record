...
def reset_history() -> None:
    ...
    `st.session_state`에 저장된 'messages' 키의 값을 빈 리스트(`[]`)로 재설정하여,
    현재까지의 모든 대화 기록을 삭제합니다. 'Chat Reset'버튼의 콜백 함수로 사용됩니다.
    """
...
def view_messages(messages: list[dict]) -> None:
    ...
    메시지 딕셔너리 리스트를 순회하며, 각 메시지의 'role'과 'content'를 사용하여
    Streamlit의 `st.chat_message` 컨테이너 내에 채팅 내용을 렌더링합니다.
...
def chat_input() -> dict | None:
    ...
    `st.chat_input`을 사용하여 사용자로부터 텍스트 입력을 기다립니다.
